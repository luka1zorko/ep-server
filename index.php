...
$urls = [
    ...
    "/^item\/add$/" => function () {
        utils::use_HTTPS();
        if (utils::isLoggedIn() && utils::isSalesman()) {itemController::addItem();}
        else {ViewHelper::redirect(BASE_URL . "items");}
    },
    "/^item\/edit$/" => function () {
        utils::use_HTTPS();
        if (utils::isLoggedIn() && utils::isSalesman()) {itemController::edit();}
        else {ViewHelper::redirect(BASE_URL . "items");}
    },
    "/^item\/delete$/" => function () {
        utils::use_HTTPS();
        if (utils::isLoggedIn() && utils::isSalesman()) {itemController::delete();}
        else {ViewHelper::redirect(BASE_URL . "items");}
    },
